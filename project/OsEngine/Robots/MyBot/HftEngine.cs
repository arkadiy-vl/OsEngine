...
namespace OsEngine.Robots.MyBot
{
    public class HftEngine : BotPanel
    {
        ...
        public List<string> NamesSecuritiesToConnect = new List<string>(); // список инструментов, которые должны быть подключены в роботе
        public List<string> NamesSecuritiesAlreadyConnect = new List<string>(); // список инструментов, уже подключенных в роботе
        ...
        public HftEngine(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            // создаем новый поток, для автоматического подключения инструментов
            Thread connectionThread = new Thread(ConnectionMethod);
        ...
        public void Save()
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(@"Engine\" + NameStrategyUniq + @"SettingsBot.txt", false))
                {
                    ...
                    writer.WriteLine(MyServerType);
                    writer.WriteLine(PortfolioNum);
                    for (int i = 0; i < NamesSecuritiesToConnect.Count; i++)
                    {
                        writer.WriteLine(NamesSecuritiesToConnect[i]);
        ...
        private void Load()
        {
            ...
            try
            {
                using (StreamReader reader = new StreamReader(@"Engine\" + NameStrategyUniq + @"SettingsBot.txt"))
                {
                    ...
                    Enum.TryParse(reader.ReadLine(), out MyServerType);
                    PortfolioNum = reader.ReadLine();
                    while (reader.EndOfStream == false) 
                    {
                        NamesSecuritiesToConnect.Add(reader.ReadLine());
                    }
        ...
        private void ServerMaster_ServerCreateEvent(IServer newServer)
        {
            ...
            newServer.NeadToReconnectEvent += NewServer_NeadToReconnectEvent;       // событие реконекта сервера
        }
...
// connect logic
        ...
        public void ConnectionMethod()
        {
            while (true)
            {
                ...
                {
                    bool isInList = false;
                    for (int j = 0; j < NamesSecuritiesAlreadyConnect.Count; j++)
                    {
                        if(NamesSecuritiesAlreadyConnect[j] == NamesSecuritiesToConnect[i])
                        {
                            isInList = true;
                            break;
                        }
                    ...
                    if(isInList == false)
                    {
                        if (ConnectThisSecurity(NamesSecuritiesToConnect[i]))
                        {
                            NamesSecuritiesAlreadyConnect.Add(NamesSecuritiesToConnect[i]);
                        }
                    }
                }
            }
        ...
        public bool ConnectThisSecurity(string securityName)
        {
            ...
            if(myServer.StartThisSecurity(securityName, new TimeFrameBuilder()) != null)
            {
                return true;
            }
        ...
        {
            NamesSecuritiesAlreadyConnect.Clear();
        }
 ...
 // bot netto position
        ...
        private void NewServer_NewMyTradeEvent(MyTrade myTrade)
        {
            ...
            // подписываемся на получение всех трейдов по инструменту (было изначально, потом перенесли в метод ConnectionThisSecurity
